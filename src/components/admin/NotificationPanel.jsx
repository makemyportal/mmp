...
const NotificationPanel = ({ notifications, clearNotification, clearAll }) => {
    ...
    return (
        <div className="relative">
            <button onClick={() => setOpen(!open)} className="relative p-2 rounded-xl hover:bg-white/5 text-gray-500 transition-colors">
                ...
                {open && (
                    <>
                        ...
                        <motion.div
                            ...
                            className="absolute right-0 top-12 z-50 w-80 sm:w-96 bg-gray-50 border border-gray-200 rounded-2xl shadow-2xl overflow-hidden"
                        >
                            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
                                <h3 className="text-sm font-bold text-gray-900">Notifications</h3>
                                {notifications.length > 0 && (
                                    <button onClick={() => { clearAll(); }} className="text-xs text-brand-primary hover:text-gray-900 font-semibold transition-colors">
                                        Clear All
                                    </button>
                                )}
                            </div>
                            <div className="max-h-80 overflow-y-auto">
                                {notifications.length === 0 ? (
                                    ...
                                    notifications.map(n => (
                                        <div key={n.id} className={`flex items-start gap-3 px-5 py-3.5 border-b border-gray-200 hover:bg-white/[0.02] transition-colors ${!n.read ? 'bg-brand-primary/5' : ''}`}>
                                            ...
                                            <button onClick={() => clearNotification(n.id)} className="text-gray-600 hover:text-gray-600 shrink-0">
