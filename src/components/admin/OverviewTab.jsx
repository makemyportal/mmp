...
const StatCard = ({ title, value, icon: Icon, trend, trendUp, color }) => (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        className="bg-gray-50 border border-gray-200 rounded-2xl p-5 hover:border-gray-200 transition-all">
    ...
    return (
        <div className="space-y-8">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-black text-gray-900 font-heading">Dashboard Overview</h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                    ...
                    <button key={action.label} onClick={() => setActiveTab(action.tab)}
                        className="flex flex-col items-center gap-2 p-4 rounded-xl border border-gray-200 hover:border-gray-200 bg-gray-50 hover:bg-gray-100 transition-all group">
                        <div className={`p-2.5 rounded-lg bg-gradient-to-br ${action.color} shadow-lg group-hover:scale-110 transition-transform`}>
                            <action.icon className="w-4 h-4 text-gray-900" />
                        </div>
                        <span className="text-xs font-bold text-gray-500 group-hover:text-gray-900 transition-colors">{action.label}</span>
                    </button>
                ))}
            </div>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-2xl overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                    ...
                    <button onClick={() => setActiveTab('orders')} className="text-xs text-brand-primary hover:text-gray-900 font-semibold flex items-center gap-1 transition-colors">
                        ...
                        <tbody className="divide-y divide-white/5">
                            {loading ? (
                                ...
                                <tr key={order.id} className="hover:bg-white/[0.02] transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-600">{order.serviceName}</td>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-2xl p-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                    {Object.entries(services.reduce((acc, s) => { acc[s.category] = (acc[s.category] || 0) + 1; return acc; }, {})).map(([cat, count]) => (
                        <div key={cat} className="flex items-center gap-3 p-3 rounded-xl bg-white border border-gray-200">
                            <span className="text-lg">{cat.split(' ')[0]}</span>
                            <div>
                                <p className="text-xs text-gray-500 leading-tight">{cat.replace(/^[^\s]+\s/, '')}</p>
                                <p className="text-sm font-bold text-gray-900">{count} services</p>
                            </div>
                        </div>
                    ))}
                </div>
