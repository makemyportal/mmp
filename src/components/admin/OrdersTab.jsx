    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-black text-gray-900 font-heading">Order Management</h2>
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto">
                    <div className="relative flex-1 sm:flex-none">
                        ...
                        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search orders..."
                            className="w-full sm:w-64 pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm text-gray-900 focus:outline-none focus:border-brand-primary transition-colors" />
                    </div>
                        ...
                        <tbody className="divide-y divide-white/5">
                            {loading ? (
                                ...
                                <tr key={order.id} className="hover:bg-white/[0.02] transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-600 max-w-[150px] truncate">{order.serviceName}</td>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-1">
                                            ...
                                            <button onClick={() => setEditingOrder(order)} className="p-2 rounded-lg text-gray-500 hover:text-gray-900 hover:bg-white/10 transition-colors" title="Manage">
                ...
                {editingOrder && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                            className="absolute inset-0 bg-white/80 backdrop-blur-sm" onClick={() => setEditingOrder(null)} />
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                            className="relative w-full max-w-lg bg-gray-50 border border-gray-200 rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
                            <div className="flex justify-between items-center p-6 border-b border-gray-200 shrink-0">
                                <h3 className="text-xl font-bold text-gray-900 font-heading">Manage Order</h3>
                                <button onClick={() => setEditingOrder(null)} className="text-gray-500 hover:text-gray-900"><X className="w-5 h-5" /></button>
                            </div>
                            <div className="p-6 space-y-5 overflow-y-auto">
                                <div className="grid grid-cols-2 gap-4">
                                    <div><p className="text-xs text-gray-500 mb-1">Customer</p><p className="text-sm font-medium text-gray-900">{editingOrder.customerName || 'N/A'}</p></div>
                                    <div><p className="text-xs text-gray-500 mb-1">Email</p><p className="text-sm text-gray-600 flex items-center gap-1"><Mail className="w-3 h-3" />{editingOrder.userEmail}</p></div>
                                    <div><p className="text-xs text-gray-500 mb-1">Service</p><p className="text-sm font-medium text-gray-900">{editingOrder.serviceName}</p></div>
                                    ...
                                    {editingOrder.phone && <div><p className="text-xs text-gray-500 mb-1">Phone</p><p className="text-sm text-gray-600 flex items-center gap-1"><Phone className="w-3 h-3" />{editingOrder.phone}</p></div>}
                                    {editingOrder.company && <div><p className="text-xs text-gray-500 mb-1">Company</p><p className="text-sm text-gray-600">{editingOrder.company}</p></div>}
                                </div>
                                {editingOrder.requirements && (
                                    <div><p className="text-xs text-gray-500 mb-1 flex items-center gap-1"><MessageSquare className="w-3 h-3" />Requirements</p>
                                        <p className="text-sm text-gray-600 bg-white p-3 rounded-xl border border-gray-200">{editingOrder.requirements}</p>
                                    </div>
                                )}
                                <div>
                                    ...
                                    <div className="space-y-2">
                                        {editingOrder.steps && editingOrder.steps.map((step, idx) => (
                                            <button key={idx}
                                                ...
                                                disabled={saving}
                                                className={`w-full flex items-center gap-3 p-3 rounded-xl border transition-all text-left text-sm ${step.completed ? 'border-emerald-500/30 bg-emerald-500/5 text-emerald-400' : 'border-gray-200 bg-white text-gray-500 hover:border-brand-primary/30 hover:text-gray-900'}`}>
