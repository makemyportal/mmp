...
const WhatsAppFloat = () => {
    ...
    return (
        <div className="fixed bottom-[84px] md:bottom-6 right-4 md:right-6 z-50">
            {/* Popup */}
            {open && (
                <div className="absolute bottom-16 right-0 bg-gray-50 border border-gray-200 rounded-2xl shadow-2xl shadow-black/50 p-4 w-72 animate-in slide-in-from-bottom-2">
                    <div className="flex items-center justify-between mb-3">
                        <h3 className="text-gray-900 font-heading font-semibold text-sm">Book on WhatsApp</h3>
                        <button onClick={() => setOpen(false)} className="text-gray-500 hover:text-gray-900 transition-colors">
                            <X className="w-4 h-4" />
                        </button>
                    </div>
                    <p className="text-gray-500 text-xs mb-3">Tap a number below to start a chat:</p>
                        ...
                        {numbers.map((item) => (
                            ...
                            >
                                <div className="w-9 h-9 rounded-full bg-green-600 flex items-center justify-center shrink-0">
                                    <span className="text-gray-900 font-bold text-sm">{item.name[0]}</span>
                                </div>
                                <div>
                                    <p className="text-gray-900 text-sm font-medium">{item.name}</p>
                                    <p className="text-gray-500 text-xs">{item.number.slice(2)}</p>
                                </div>
                            </a>
                        ))}
                    </div>
