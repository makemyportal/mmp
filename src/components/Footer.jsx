                    ...
                    <div className="space-y-6">
                        <Link to="/" className="flex items-center gap-2 group w-fit">
                            <div className="relative flex items-center justify-center w-8 h-8 rounded-lg bg-gradient-to-br from-brand-primary to-brand-secondary overflow-hidden">
                                <Rocket className="text-white w-4 h-4 z-10" />
                            </div>
                                ...
                                <li key={link.name}>
                                    {link.isAnchor ? (
                                        <a
                                            ...
                                            className="text-gray-400 text-sm hover:text-brand-primary transition-colors flex items-center gap-2 group cursor-pointer"
                                        >
                                            <span className="w-1.5 h-1.5 rounded-full bg-white/20 group-hover:bg-brand-primary transition-colors" />
                                            {link.name}
                                        </a>
                                    ) : (
                                        <Link to={link.path} className="text-gray-400 text-sm hover:text-brand-primary transition-colors flex items-center gap-2 group">
                                            <span className="w-1.5 h-1.5 rounded-full bg-white/20 group-hover:bg-brand-primary transition-colors" />
                                            {link.name}
                                        </Link>
                                    )}
                    ...
                    <div>
                        ...
                        <ul className="space-y-4">
                            {['Web Development', 'EdTech Platforms', 'Brand Identity', 'AI Integration', 'Tech Consulting'].map((service) => (
                                <li key={service}>
                                    <Link to="/services" className="text-gray-400 text-sm hover:text-brand-secondary transition-colors flex items-center gap-2 group">
                    ...
                    <div>
                        ...
                        <ul className="space-y-4 text-sm text-gray-400">
                            ...
                            <li className="flex items-center gap-3">
                                ...
                                <a href={`tel:${settings.phone || ''}`} className="hover:text-white transition-colors">{settings.phone || 'Contact Us'}</a>
                            </li>
                            <li className="flex items-center gap-3">
                                <Mail className="w-5 h-5 text-brand-accent shrink-0" />
                                <a href={`mailto:${settings.contactEmail || 'hello@makemyportal.com'}`} className="hover:text-white transition-colors">{settings.contactEmail || 'hello@makemyportal.com'}</a>
                            </li>
                        </ul>
                    </div>
                </div>
