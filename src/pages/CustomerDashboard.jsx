    ...
    <AnimatePresence>
        {message && (
            <motion.div
                ...
                exit={{ opacity: 0, y: 50 }}
                className="fixed bottom-6 right-6 z-50 bg-gray-50 border border-brand-primary/30 text-white px-5 py-3 rounded-xl shadow-2xl flex items-center gap-3 max-w-sm"
            >
                <span className="text-sm font-medium">{message}</span>
                <button onClick={onClose} className="text-gray-500 hover:text-gray-900 transition-colors shrink-0">
                    <X className="w-4 h-4" />
                </button>
            </motion.div>
        )}
                ...
                <div className="glass-panel p-8 rounded-[2rem] border border-gray-200 bg-gradient-to-br from-gray-50 to-white shadow-2xl relative overflow-hidden">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
                        <div>
                            ...
                            <h1 className="text-4xl font-black font-heading mb-2 text-gray-900">Welcome back, {userData?.name || currentUser?.email?.split('@')[0]}</h1>
                            <p className="text-gray-500 font-medium">Manage your digital assets, track project progress, and view invoices.</p>
                        </div>
                        <div className="flex gap-3">
                            <button onClick={() => showToast("Profile Editor coming soon — stay tuned!")} className="px-5 py-2.5 rounded-xl border border-gray-200 hover:bg-white/5 transition-colors font-bold text-sm text-gray-900">
                                Edit Profile
                            </button>
                            <Link to="/services" className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-brand-primary to-blue-600 hover:opacity-90 transition-opacity font-bold text-sm text-gray-900 shadow-lg">
                                New Order
                            </Link>
                        </div>
                    </div>
                            ...
                            {(activeTab === 'subscriptions' || activeTab === 'support') && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center h-full min-h-[400px] text-center opacity-60">
                                    <div className="w-16 h-16 rounded-2xl bg-white/5 flex items-center justify-center mb-4">
                                        {activeTab === 'subscriptions' ? <CreditCard className="w-8 h-8 text-gray-500" /> : <LifeBuoy className="w-8 h-8 text-gray-500" />}
                                    </div>
                                    ...
                                    {activeTab === 'support' && (
                                        <a href="https://wa.me/917017978807?text=Hi, I need support for my Makemyportal account." target="_blank" rel="noopener noreferrer" className="px-6 py-3 rounded-xl bg-white/10 hover:bg-white/20 text-gray-900 font-bold transition-colors">
